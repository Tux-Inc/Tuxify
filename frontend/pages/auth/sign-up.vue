...
const validate = (state: any): FormError[] => {
    ...
    if (!state.password2) errors.push({ path: "confirmPassword", message: "Required" });
    return errors;
};
...
<template>
    <div class="h-screen w-full flex flex-col gap-4 items-center justify-center">
        <NuxtLink to="/">
            <svg class="h-16 fill-primary dark:fill-base-light" viewBox="0 0 2262 782"
        ...
        <UCard class=" w-full sm:w-1/2 md:w-1/3">
            ...
            >
                ...
                <UFormGroup :label="i18n.t('auth.form.password')" name="password">
                    <UInput v-model="state.password1" type="password" placeholder="********" />
                </UFormGroup>
                <UFormGroup :label="i18n.t('auth.form.confirmPassword')" name="confirmPassword">
                    <UInput v-model="state.password2" type="password" placeholder="********" />
                </UFormGroup>
                <UButton :label="i18n.t('auth.button.next')" block size="lg" trailing-icon="i-heroicons-arrow-right"
                         type="submit" :loading="isLoading" />
                <UButton :label="i18n.t('auth.button.back')" block size="lg" icon="i-heroicons-arrow-left"
                         to="/auth/sign-in" color="primary" variant="ghost" />
            </UForm>
        </UCard>
    </div>
</template>
